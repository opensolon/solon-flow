<template>
  ...
  let formData = reactive({ id: null, title: null, task: null, when: null,meta:{
            '$for':'',
            '$in': ''
          }})
  ...
  function onChange() {
      _currentEditNode.setData({ id:formData.id,title: formData.title, task: formData.task, when: formData.when,meta:formData.meta },{
        overwrite:true
      })
