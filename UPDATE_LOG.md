...
* 
* 优化 多图执行时，在子图内停止，入口又是根图。如何通过 NodeTrace 恢复状态 ???
  * 记录一个栈？从第一个节点逐步恢复
  * 优化：如果两节点同图，则跳过上个节点，直接用下个节点恢复
  * solon-ai-agent 通过定制方式（树型记录器）解决了
...
* 调整 `solon-flow` FlowInterceptor:doIntercept 更名为 FlowInterceptor，并标为 default（扩展时语义清晰，且不需要强制实现）
